...
// Names of a secure and an insecure keysize length (1024) are stored in an object's field by using the Identity function. The secure one is then used to finalize the keypair.
async function main() {
    try {
        ...
        const { publicKey, privateKey } = await util.promisify(crypto.generateKeyPair)("rsa", keysizeclass.size2);
