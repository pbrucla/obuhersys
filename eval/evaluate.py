...
def main():
    ...
    parser.add_argument(
        "--wait",
        help="wait for confirmation after each testcase",
        action="store_true",
    )
    ...
    for meta_yaml in dataset.rglob("*.yml"):
        ...
        for test_case in meta:
            ...
            os.environ["OBU_TESTCASE"] = str(
                test_file.relative_to(dataset).with_suffix("")
            )
            ...
            ret_secure = any(
                "yes" in line and key in line for line in errs.decode().split("\n")
            )
            ret_insecure = any(
                "no" in line and key in line for line in errs.decode().split("\n")
            )
            if ret_secure == ret_insecure:
                print(
                    "❌",
                    test_file.relative_to(dataset),
                    "(both yes and no output)" if ret_secure else "(no output)",
                )
            else:
